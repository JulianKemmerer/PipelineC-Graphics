...
#define RSQRT(x) (1.0/std::sqrt(float(x))) //float_rsqrt(x)
#define SQRT(x) ((float)1.0/RSQRT(x))
#define FABS(x) std::fabs((float)x) //float_uint32(float_31_0(x) & 0x7FFFFFFF)
...
float float_rsqrt( float number ) //reports 16.56753756689143 MHz
{
  ...
  float x2 = number * 0.5;
  float conv_f = float_uint32(0x5f3759df - (float_31_0(number) >> 1));
...
class fp32
{
  ...
  static const uint32_t mulmask = ~((1 << (23-13))-1); //use just 17 bit of the 23 bit mantissa
  static const uint32_t divmask = ~((1 << (23-13))-1);
  static const uint32_t addmask = ~((1 << (23-13))-1); //min 12
...
#if 0
  fp32(float a) : f(float_uint32(float_31_0(a) & assignmask)) { }
