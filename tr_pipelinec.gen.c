...
color background_color_alt(screen_coord_t x, screen_coord_t y, uint16_t frame, uint16_t off, coord_type z)
{
...
color render_floor_alt(screen_coord_t x, screen_coord_t y, coord_type px, coord_type py, coord_type pz, color c)
{
  ...
  if(((fixed_neq(y, fixed_make_from_double((double)0.)))!=0) & ((fixed_is_negative(y) == fixed_is_negative(py))!=0)) {
    ...
    if(!(fixed_lt(hole_d, fixed_make_from_int(0)))) {
      ...
      if(((fixed_to_short(inv_y)) >> 9) == 0) {
        c = ((u ^ v) & 1) ? K_plane_color2 : K_plane_color1;
        
        if(fixed_lt(hole_d, fixed_make_from_double((double).1))) c = K_floor_difusse;
      }
      drawfog = 1;
    }
  }
  ...
  if(drawfog) c = color_select(fixed_abs(y), c, fixed3_make_from_const_fixed3(K_fog_color));
  return c;
}
...
color render_pixel_internal_alt(screen_coord_t x, screen_coord_t y, scene_t scene, scene_colors_t colors)
{
  ...
  color c = background_color_alt(x, y, scene.frame, -fixed_to_short(scene.plane.center.x), fixed_mul(dz, fixed_make_from_double((double).5 / (double)30.)));
  bool drawfloor = 1;
