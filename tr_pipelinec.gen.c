...
hit_out sphere_hit(bool hit, vec3 center, point_and_dir hitin, float t, float diff)
{
  ...
  {
    hitout.hit.orig = float3_add(hitin.orig, float3_mul_float(hitin.dir, t));
...
color sphere_effect(uint16_t frame, scene_colors_t colors, sphere_t s, hit_out hit, material_t hit_material)
{
...
color shade(scene_t scene, scene_colors_t colors, color background, vec3 dir, hit_out hit, material_t hit_material, color_type minfog)
{
...
color cast_ray(scene_t scene, scene_colors_t colors, point_and_dir hitin)
{
  ...
  return shade(scene, colors, bfog, hitin.dir, hitout, hit_material, planehit ? mix : fixed_make_from_double((double)0.));
}
...
pixel_t render_pixel(uint16_t i, uint16_t j, scene_t scene)
{
  ...
  screen_coord_t x = fixed_shr(fixed_make_from_short(cx), (FRAME_WIDTH < 1024 ? 10 : 11));
  screen_coord_t y = fixed_shr(fixed_make_from_short(cy), (FRAME_WIDTH < 1024 ? 10 : 11));
  pixel_t pix;
